...
int main() {
    ...
    cout << "Enter plaintext block in hex (32 hex digits): ";
    string plainHex;
    cin >> plainHex;
    for (int i = 0; i < 16; i++) {
        string byteStr = plainHex.substr(i*2, 2);
        plaintext[i] = (uint8_t)strtol(byteStr.c_str(), nullptr, 16);
    }
    
    AES aes(key, keySize);
    cout << "Plaintext:  ";
    aes.printBlock(plaintext);
...
Enter plaintext block in hex (32 hex digits): 112233445566778899AABBCCDDEEFF00
